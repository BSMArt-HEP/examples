...
Good starting points are the toy model scans [Random](Toy/random_rosen.json), [CMAES](Toy/cmaes_rosen.json) and [CMAES_ND](Toy/cmaesnd_rosen.json): these run as-is without any model, Les Houches file or configuration, although for the latter two the cmaes (and pyod for ND) package(s) must be installed.